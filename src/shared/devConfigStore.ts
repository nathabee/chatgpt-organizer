...
const STORAGE_KEY = "cgo.devConfig";
...
export type DevConfig = {
  ...
  failureLogsPerRun: number;   // per-run spam cap in tabs
};
...
function normalize(raw: any): DevConfig {
  const r = raw || {};
  return {
    traceScope: !!r.traceScope,
    stopAfterOutOfScopeProjects: clampInt(r.stopAfterOutOfScopeProjects ?? DEV_CONFIG_DEFAULTS.stopAfterOutOfScopeProjects, 0, 5000, DEV_CONFIG_DEFAULTS.stopAfterOutOfScopeProjects),
